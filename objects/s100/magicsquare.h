...
public:
  ...
  int getRC(int row, int column) const;

  magicsquare &operator=(const magicsquare &ms);
  friend ostream &operator<<(ostream &os, const magicsquare &ms);

  magicsquare(int num);
  magicsquare(const magicsquare &ms);
  ~magicsquare();
